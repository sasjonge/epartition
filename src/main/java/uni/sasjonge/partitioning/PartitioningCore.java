...
//        }
        ...
        switch (ax.getAxiomType().toString()) {
            ...
            case "AnnotationAssertion":
                ...
                if (annot.getSubject().asIRI().isPresent()) {
                    String vertex = OntologyDescriptor.getCleanNameOWLIRI(annot.getSubject().asIRI().get());

                    if (g.containsVertex(vertex)) {
                        edgeReference.set(createLoopEdge(g, vertex));
                        if (edgeReference.get() == null) {
                            System.err.println("Edgereference is null");
                        }
                    }
                } else {
                    System.err.println("Missing annotation subject: " + annot.getSubject().toString());
                }
     ...
     * AnnotationPropertyRangeOf and AnnotationPropertyDomainOf Axioms which contain the
     * property
     *
     * @param ontology
